...
**crm2 coexistence (retired).** crm2 was delinked from the shared prod DB on 2026-06-16; the coexistence constraint is retired. The legacy↔dotted permission bridge is still pinned exhaustively by `tests/Feature/Permissions/PermissionAliasBridgeTest.php` (runs in CI).
...
- **Database migrations (canonical flow + hard invariant).** Schema/permission changes are ALWAYS Laravel migration files here — never ad-hoc `ALTER TABLE` or hand-inserted rows on prod. Flow: (1) author the migration locally (additive + reversible); (2) run it on the local `crm` DB (MariaDB :3307) via `php artisan migrate`; (3) commit so it travels with the code (shahid → backend → origin → server); (4) on deploy, `migrate --force` (with `/opt/alt/php84/usr/bin/php`) on prod runs the SAME files local already ran. **INVARIANT: the local `crm` DB is always ≥ prod and must NEVER be behind** — a migration only reaches prod if it is in `origin` AND was run + coexistence-tested locally first; no ad-hoc prod schema/permission changes, ever. Before any backend deploy, pull `origin backend` into local and `migrate` locally so local has everything headed to / already on prod (catches drift). (A 2026-06-03 check found prod's `migrations` log ahead of local/`origin` by ~44 permission-migrations — before relying on local==prod, re-verify current parity with `scripts/parity-check.sh`; see `project_local_vs_production_db`.) Memory: `project_local_vs_production_db`, `feedback_crm3_must_not_break_crm2`.
- **Backend team-sync process (ALWAYS run the COMPLETE process — never skip a step; user directives 2026-06-04). Runs AUTONOMOUSLY: notify progress, don't ask per step — STOP only for go-live/prod-deploy and genuine risk (conflict, failing test, network failure, prod schema change, ambiguous/destructive). See `project_team_branch_workflow`.** THE ONE RULE: nothing goes live unless it's pushed to `origin` first — no laptop→server shortcuts. Roles: Shahid `shahid`, Gulraiz `gulraiz`; live branch = `backend`; `origin` is the shared source of truth. **① Sync / before starting work:** `git fetch origin backend gulraiz` → merge `origin/gulraiz` into `local shahid` → `php artisan migrate` → **full test suite must pass — run `php -d memory_limit=-1 vendor/bin/pest` (NOT `php artisan test`: its subprocess ignores `-d` and OOMs at the default 128M ~⅔ through; ~1574 tests, 157s).** **② Before going live:** push `shahid`→`origin shahid` → merge into `backend` → **run full pest suite (`php -d memory_limit=-1 vendor/bin/pest`, must pass — test at EVERY merge, not only the sync)** → push `backend`→`origin backend` → THEN deploy (push `backend` local→server); never deploy anything not already on `origin`. **③ Safety net:** periodically refresh local DB from prod so it never silently falls behind. Memory: `project_team_branch_workflow`, `project_deploy_pipelines`.
