...
- **Frontend deploy flow (the sibling SPA repo — for cross-repo awareness).** Frontend release branch is **`main`** (no `frontend` branch). Same first steps (code on `local shahid` → push `origin shahid` → merge into `local main` → push `origin main`, after pulling `origin main`). But **deploy differs: it is build-and-copy, NOT a git push** — the crm3 SPA on the server is static files with no git. Checkout `local main`, `npm run build` (committed `.env.production` → `https://api.cutera.pk`), `tar` `dist/` **excluding `.htaccess`**, scp up, extract into `~/domains/crm3.cutera.pk/spa` **preserving the server `.htaccess`** (SPA routing + Basic-Auth gate), verify crm3 serves the new hash + `crm2.cutera.pk/login` = 200. See `project_deploy_pipelines` memory.
