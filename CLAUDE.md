...
**Standards live in two charters — read before writing code/tests:** `GUIDELINES.md` (coding standards) and `TESTING.md` (the test-sync system), companions to the frontend repo's copies. This file is project context + the hard constraints below; those charters hold the detailed standards.
...
**crm2 coexistence (hard constraint).** crm2 (legacy Blade, branch `production`) and crm3 (this backend's `backend` integration line + the SPA) run **on one shared production DB**, and will until crm2 is deliberately disconnected. A change here must **never break crm2**: don't delete/rename a legacy snake_case permission, strip a legacy role grant, or drop/rename a shared column / response shape crm2 reads — crm2 has **no** permission bridge, so it would silently 403/500. Mirror prod locally once with `scripts/setup-local-coexistence.sh`; before shipping any DB / permission / shared-schema change run `scripts/coexistence-check.sh` (must PASS). The legacy↔dotted bridge is pinned exhaustively by `tests/Feature/Permissions/PermissionAliasBridgeTest.php` (runs in CI). Breaking a crm2 contract is a deliberate "disconnect crm2" decision — surface it, never silent. See `docs/LOCAL-COEXISTENCE.md`.

**Server access (SSH) — READ-only is pre-authorized; WRITE needs explicit per-action approval.** Read-only actions on the live server (diagnostics, log tails, `grep`/`cat`/`ls`, `SELECT` queries, read-only `php artisan` inspection, reading `.env`/config) — just run them when they help and say what you ran; no need to ask first. WRITE/mutating actions (`.env`/config edits, `config:clear`/cache rebuilds, `php artisan migrate` or anything that changes DB/files/state, deploys, restarts) — propose the exact command + what it does, then wait for an explicit yes for *that* action; never assume a write from a prior "yes", a general "go ahead", or a task instruction. If unsure whether something is read or write, treat it as a write. All development is done **locally**; deploy by git-pushing `local backend` → the server over SSH (that git push *is* the pipeline — see Workflow protocol), never by hand-editing files on the box. (An assumed-approval `config:clear` broke crm3 login once — that's why writes stay gated.)
...
- **Full charter: `TESTING.md`** (the test-sync system; companion to `../frontend/TESTING.md`) — read it before writing tests. It holds the Definition of Done, the change→test map (what test you owe for what change), and the waiver policy; the gates are `scripts/test-sync-check.sh` (in-session/CI) and the CI suite. Run the suite via `php -d memory_limit=-1 vendor/bin/pest` (`php artisan test` OOMs at 128M). The rules below are the highlights.
...
- **Backend deploy flow (canonical — always follow this order).** `backend` is the release branch that mirrors production. (1) Code on `local shahid` (working branch). (2) Push `local shahid` → `origin shahid` (cloud backup + team visibility). (3) Merge `shahid` into `local backend`, then push `local backend` → `origin backend` to keep the cloud release copy in sync. (4) Deploy by git-pushing `local backend` → the live server over SSH — the server **cannot** pull from Bitbucket, so this local→server git push *is* the pipeline (a git push, not manual file edits on the box) — then `migrate --force` + clear caches with `/opt/alt/php84/usr/bin/php`. `local backend` is the hub: it pushes to BOTH `origin backend` (Bitbucket, team/backup) and the live server (deploy); Bitbucket is NOT a pass-through to the server. Every git/SSH step still needs the per-action approval required above. See `project_deploy_pipelines` memory.
- **Frontend deploy flow (sibling SPA repo — cross-repo awareness).** Frontend repo branches: `shahid`/`gulraiz` → `frontend`; **there is no `crm3` or `main` branch — `frontend` is the only integration + release branch.** crm3 is **live in prod** (coexisting with crm2 since ~2026-05-30). **Deploy = build-and-copy, NOT a git push** (the crm3 SPA on the server is static files, no git): run the gated `bash scripts/deploy-crm3.sh` from a clean `local frontend` == `origin frontend` — it builds (committed `.env.production` → `https://api.cutera.pk`), tars `dist/` **excluding `.htaccess`**, scps up, extracts into `~/domains/crm3.cutera.pk/spa` **preserving the server `.htaccess`** (SPA routing + Basic-Auth gate), stamps the build commit into `dist/version.json`, and verifies `crm2.cutera.pk/login` = 200. Never build/deploy from `shahid`. See `project_deploy_pipelines`, `project_backend_integration_branch`.
- **Database migrations (canonical flow + hard invariant).** Schema/permission changes are ALWAYS Laravel migration files here — never ad-hoc `ALTER TABLE` or hand-inserted rows on prod. Flow: (1) author the migration locally; (2) run it on the local `crm` DB (MariaDB :3307) via `php artisan migrate`; (3) coexistence-test locally — `scripts/coexistence-check.sh` must PASS (additive only; never drop/rename a legacy permission/column/response shape crm2 reads); (4) commit so it travels with the code (shahid → backend → origin → server); (5) on deploy, `migrate --force` (with `/opt/alt/php84/usr/bin/php`) on prod runs the SAME files local already ran. **INVARIANT: the local `crm` DB is always ≥ prod and must NEVER be behind** — a migration only reaches prod if it is in `origin` AND was run + coexistence-tested locally first; no ad-hoc prod schema/permission changes, ever. Before any backend deploy, pull `origin backend` into local and `migrate` locally so local has everything headed to / already on prod (catches drift). (A 2026-06-03 check found prod's `migrations` log ahead of local/`origin` by ~44 permission-migrations — before relying on local==prod, re-verify current parity with `scripts/parity-check.sh`; see `project_local_vs_production_db`.) Memory: `project_local_vs_production_db`, `feedback_crm3_must_not_break_crm2`.
- **Backend team-sync process (ALWAYS run the COMPLETE process — never skip a step; user directives 2026-06-04). Runs AUTONOMOUSLY: notify progress, don't ask per step — STOP only for go-live/prod-deploy and genuine risk (conflict, failing test/coexistence, network failure, prod schema change, ambiguous/destructive). See `project_team_branch_workflow`.** THE ONE RULE: nothing goes live unless it's pushed to `origin` first — no laptop→server shortcuts. Roles: Shahid `shahid`, Gulraiz `gulraiz`; live branch = `backend`; `origin` is the shared source of truth. **① Sync / before starting work:** `git fetch origin backend gulraiz` → merge `origin/gulraiz` into `local shahid` → `php artisan migrate` → `scripts/coexistence-check.sh` (crm2 safe) → **full test suite must pass — run `php -d memory_limit=-1 vendor/bin/pest` (NOT `php artisan test`: its subprocess ignores `-d` and OOMs at the default 128M ~⅔ through; ~1574 tests, 157s).** **② Before going live:** push `shahid`→`origin shahid` → merge into `backend` → **run full pest suite (`php -d memory_limit=-1 vendor/bin/pest`, must pass — test at EVERY merge, not only the sync)** → push `backend`→`origin backend` → THEN deploy (push `backend` local→server); never deploy anything not already on `origin`. **③ Safety net:** periodically refresh local DB from prod so it never silently falls behind. Memory: `project_team_branch_workflow`, `project_deploy_pipelines`.
...
- **Full charter: `GUIDELINES.md`** (backend companion to `../frontend/GUIDELINES.md`) — read it before writing code. The rules below are the highlights; the charter holds the complete standing disciplines, reuse/security/DB rules, and the machine-checkable subset enforced by `scripts/guidelines-check.sh` in CI.
