...
- **Frontend deploy flow (sibling SPA repo — cross-repo awareness).** Frontend repo branches: `shahid`/`gulraiz` → `frontend` (integration) → `crm3` (deploy); **`main` was DELETED — there is no `main`**. **Deploy = build-and-copy, NOT a git push** (the crm3 SPA on the server is static files, no git): `npm run build` (committed `.env.production` → `https://api.cutera.pk`), `tar` `dist/` **excluding `.htaccess`**, scp up, extract into `~/domains/crm3.cutera.pk/spa` **preserving the server `.htaccess`** (SPA routing + Basic-Auth gate), verify new hash + `crm2.cutera.pk/login` = 200. (As of 2026-06-03 `frontend`/`crm3` are caught up to `shahid`; live prod is still the manual `shahid` build until CI deploy is wired.) See `project_deploy_pipelines`, `project_backend_integration_branch`.
- **Database migrations (canonical flow + hard invariant).** Schema/permission changes are ALWAYS Laravel migration files here — never ad-hoc `ALTER TABLE` or hand-inserted rows on prod. Flow: (1) author the migration locally; (2) run it on the local `crm` DB (MariaDB :3307) via `php artisan migrate`; (3) coexistence-test locally — `scripts/coexistence-check.sh` must PASS (additive only; never drop/rename a legacy permission/column/response shape crm2 reads); (4) commit so it travels with the code (shahid → backend → origin → server); (5) on deploy, `migrate --force` (with `/opt/alt/php84/usr/bin/php`) on prod runs the SAME files local already ran. **INVARIANT: the local `crm` DB is always ≥ prod and must NEVER be behind** — a migration only reaches prod if it is in `origin` AND was run + coexistence-tested locally first; no ad-hoc prod schema/permission changes, ever. Before any backend deploy, pull `origin backend` into local and `migrate` locally so local has everything headed to / already on prod (catches drift). (Known live violation 2026-06-03: 44 permission-migrations logged on prod but absent from local + `origin`; prod `migrations` table = 409 vs ~75 files on disk — reconciliation pending, see `project_prod_schema_baseline_gap`.) Memory: `project_local_vs_production_db`, `feedback_crm3_must_not_break_crm2`.
