<div>
    <table>
        <tr style="background-color: #3F4254; font-size: 14px;">
            ...
            <th style="padding: 10px 15px 10px 15px; color: #fff;">Treatment</th>
            <th style="padding: 10px 15px 10px 15px; color: #fff;">Conversion date</th>
        ...
        @foreach($patient_data as $patient)
            ...
            <tr style="font-size: 14px; background-color: #f6f6f6;">
                ...
                <td style="padding: 10px 5px 10px 5px;">{{$patient['is_treatment'] == 1 ? 'No Show' : 'Not Booked'}} </td>
                <td style="padding: 10px 5px 10px 5px;">{{ Carbon\Carbon::parse($patient['created_at'])->format('Y-m-d') }}</td>
