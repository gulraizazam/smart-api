...
<head>
    <meta charset="utf-8" />
    <title>Invoice — {{ $account->name ?? 'Cutera Aesthetics' }}</title>
    <meta content="Cutera Aesthetics — Medical Spa Invoice" name="description" />
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .feedback-box .feedback-line:last-child {
            border-bottom: none;
        }
...
<div class="invoice-pdf">
    ...
    {{-- Cancellation watermark stays optional / commented as before. --}}
    @if($invoicestatus->slug == 'cancelled')
        {{--<img src="{{ url('metronic/assets/pages/media/invoice/cancld.png') }}" style="width: 20%;text-align: center;padding-left:43%;" alt=""/>--}}
    @endif
    ...
    <div class="brand-row">
        ...
        <div class="invoice-mark">
            ...
            <p class="num">No. <strong>#{{ $Invoiceinfo->id }}</strong></p>
        </div>
    </div>
    ...
    <div class="meta-strip">
        <div class="meta-cell">
            ...
            <div class="meta-value">
                {{ \Carbon\Carbon::parse($Invoiceinfo->created_at)->format('F j, Y') }}
                <span class="sub">{{ \Carbon\Carbon::parse($Invoiceinfo->created_at)->format('h:i a') }}</span>
            </div>
        </div>
        <div class="meta-cell">
            ...
            <tr>
                ...
                <td class="num-col">
                    @if($appointment_info?->appointment_type_id == 2 && isset($service_price_display))
                        {{-- Treatment invoice: actual_price from package_services or services.price. --}}
                ...
                <td>{{ $discount?->name ?? '—' }}</td>
                <td class="num-col">{{ number_format($Invoiceinfo->tax_including_price) }}</td>
                <td class="num-col">
                ...
                <td class="num-col">{{ $Invoiceinfo->tax_percentage }}</td>
                <td class="num-col">{{ $Invoiceinfo->tax_price }}</td>
                <td class="num-col total-col">{{ number_format($Invoiceinfo->tax_including_price) }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="totals">
        ...
        <div class="totals-card">
            <div class="totals-row">
                ...
                <div class="val">{{ number_format($Invoiceinfo->tax_exclusive_serviceprice) }}</div>
            </div>
            <div class="totals-row">
                <div class="lbl">Tax ({{ $Invoiceinfo->tax_percentage }}%)</div>
                <div class="val">{{ number_format((float) $Invoiceinfo->tax_price) }}</div>
            </div>
            <div class="totals-row grand">
                <div class="lbl">Grand Total</div>
                <div class="val">Rs. {{ number_format((float) $Invoiceinfo->total_price) }}</div>
            </div>
        </div>
    </div>

    {{-- ────────── Satisfaction rating ────────── --}}
    <div class="section-title">How satisfied are you with the quality of services provided?</div>
    <div class="rating">
        <div class="rating-label">Low</div>
        <div class="rating-cells">
    ...
    <div class="signatures">
        ...
        <div class="sig">
            ...
            <span class="name">{{ $appointment_info?->doctor?->name ?? '—' }}</span>
        </div>
    </div>
</div>
