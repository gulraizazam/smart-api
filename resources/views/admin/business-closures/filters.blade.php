<div class="mb-7">
    <div class="row align-items-end">
        <div class="col-md-3 my-2">
            <label class="mb-2">Location:</label>
            <select class="form-control datatable-input" id="search_location_id" data-col-index="1">
                <option value="">Select</option>
            </select>
        </div>
        <div class="col-md-3 my-2">
            <label class="mb-2">Start Date:</label>
            <input type="text" class="form-control datatable-input" id="search_start_date" placeholder="Select" readonly data-col-index="2">
        </div>
        <div class="col-md-3 my-2">
            <label class="mb-2">End Date:</label>
            <input type="text" class="form-control datatable-input" id="search_end_date" placeholder="Select" readonly data-col-index="3">
        </div>
        <div class="col-md-3 my-2">
            <a href="javascript:void(0);" class="btn btn-primary px-6 font-weight-bold" id="kt_search">
                Search
            </a>
            <a href="javascript:void(0);" class="btn btn-secondary px-6 font-weight-bold" id="kt_reset">
                Reset
            </a>
        </div>
    </div>
</div>
