#!/usr/bin/env python3
"""Deploy PHP files to portal.alluraesthetics.pk (the PROD backend) via SFTP.

Sibling of scripts/deploy-demo-backend.py — same shape, different target:
...
The Hostinger box can't pull from GitHub (no cached creds / deploy key), so
the deploy is: SFTP-upload the changed files, PHP-lint them on the server,
purge any Laravel bootstrap/cache/*.php so the new code takes effect.

Usage:
  python scripts/deploy-backend.py                        # files from HEAD
  python scripts/deploy-backend.py --commit HEAD~1        # files from a specific commit
  python scripts/deploy-backend.py --files a.php b.php    # explicit list
  python scripts/deploy-backend.py --dry-run              # show plan, don't upload

Credentials are read from scripts/deploy-backend.env (git-ignored) — see
scripts/deploy-backend.env.example. Required keys: SSH_HOST, SSH_PORT,
SSH_USER, SSH_PASS, REMOTE_ROOT.

Requires: Python 3.8+ and `pip install paramiko`.

Note on Demo* seeders: this script SKIPS database/seeders/Demo* — demo data
does not belong in prod. deploy-demo-backend.py is the opposite (it INCLUDES
them, since they're the point of the demo stack).
"""
...
def main() -> int:
    ...
    p.add_argument("--allow-any-branch", action="store_true",
                   help="Skip the on-backend-branch gate (use for hot-fixing from a topic branch)")
    args = p.parse_args()

    # Gate: refuse to deploy from anything but backend unless overridden.
    br = current_branch()
    if br != "backend" and not args.allow_any_branch:
        sys.exit(
            f"REFUSING: on branch '{br}', not 'backend'. Prod deploys should come from "
            "the 'backend' branch so what's live matches what's in origin/backend. "
    ...
    # PHP only; skip test / demo-seed / CI / local-deploy files.
    SKIP_PREFIX = ("tests/", "database/seeders/Demo", "scripts/", ".github/")
    files = [
        f for f in files
