...
import { LeadSourceFormDialog } from '@/components/lead-sources/lead-source-form-dialog';
...
export default function LeadSourcesPage() {
  ...
  const list = useQuery({
    queryKey: ['lead-sources', 'datatable', page, perPage, debouncedSearch, statusFilter],
    placeholderData: keepPreviousData,
    queryFn: () =>
      api.post<DatatableResponse>(
        '/api/lead_sources/datatable',
        {
  ...
  const toggleStatus = useMutation({
    mutationFn: ({ id, active }: { id: number; active: 0 | 1 }) =>
      api.post('/api/lead_sources/status', { id, status: active }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['lead-sources', 'datatable'] }),
  });

  const deleteRow = useMutation({
    mutationFn: (id: number) => api.delete(`/api/lead_sources/${id}`),
    onSuccess: () => {
  ...
  return (
    <div className="mx-auto max-w-[1400px] space-y-3 px-4 py-4 sm:px-6 lg:px-8">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto">
          <Button size="sm" onClick={() => setAddOpen(true)}>
            ...
            <span className="hidden xs:inline sm:inline">Add source</span>
      ...
      <Card className="overflow-hidden">
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-b border-hairline">
                <TableHead className="w-[60%] pl-4 text-[11px] font-semibold uppercase tracking-wide text-fg-subtle">Name</TableHead>
                <TableHead className="w-[20%] text-[11px] font-semibold uppercase tracking-wide text-fg-subtle">Status</TableHead>
            ...
            <TableBody>
              {list.isLoading && Array.from({ length: 6 }).map((_, i) => (
                <TableRow key={`sk-${i}`}>
                  <TableCell className="py-2.5 pl-4"><Skeleton className="h-4 w-full max-w-[240px]" /></TableCell>
                    ...
                    <div className="flex items-center justify-end gap-0.5">
                      {(perms.active || perms.inactive) && (
                        <RowIconAction
                          ...
                          onClick={() =>
                            toggleStatus.mutate({ id: row.id, active: row.active ? 0 : 1 })
                          }
                        />
                      )}
                      {perms.edit !== false && (
        ...
        <div className="divide-y divide-hairline/70 md:hidden">
          {list.isLoading && Array.from({ length: 4 }).map((_, i) => (
            <div key={`m-sk-${i}`} className="p-4"><Skeleton className="h-10 w-full" /></div>
          ))}
          {!list.isLoading && rows.map((row) => (
        ...
        {!list.isLoading && rows.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <h3 className="mt-4 text-[14px] font-medium text-fg">
              {hasFilters ? 'No lead sources match your filters' : 'No lead sources yet'}
            </h3>
            <p className="mt-1 max-w-xs text-[12.5px] text-fg-muted">
              {hasFilters
                ? 'Try clearing filters or adjusting your search.'
                : 'Add your first lead source to start tracking where leads come from.'}
            </p>
            {hasFilters && (
        ...
        {list.error && (
          <div className="border-t border-hairline bg-danger-soft px-4 py-3 text-[12.5px] text-danger">
            Couldn’t load lead sources: {(list.error as Error).message}
          </div>
        )}
      ...
      <LeadSourceFormDialog mode="create" open={addOpen} onOpenChange={setAddOpen} />
      <LeadSourceFormDialog
        mode="edit"
        sourceId={editId ?? undefined}
        open={editId !== null}
            ...
            <DialogDescription>
              {deleteTarget && (
                <>
                  ...
                  The server blocks deletion if any leads still reference this source.
                </>
              )}
            </DialogDescription>
          </DialogHeader>
