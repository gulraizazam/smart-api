...
{
    $total = 0;
    $report_data = [['Task', 'Hours per Day']];

    // Define date ranges based on the period
    $dateRanges = [
        'today' => [
            fn() => [Carbon::now()->format('Y-m-d'), Carbon::now()->format('Y-m-d')]
        ],
        'yesterday' => [
            fn() => [Carbon::now()->subDay()->format('Y-m-d'), Carbon::now()->subDay()->format('Y-m-d')]
        ],
        'last7day' => [
            fn() => [Carbon::now()->subDays(6)->format('Y-m-d'), Carbon::now()->format('Y-m-d')]
        ],
        'week' => [
            fn() => [Carbon::now()->startOfWeek()->format('Y-m-d'), Carbon::now()->endOfWeek()->format('Y-m-d')]
        ],
        'thisMonth' => [
            fn() => [Carbon::now()->startOfMonth()->format('Y-m-d'), Carbon::now()->endOfMonth()->format('Y-m-d')]
        ],
        'lastMonth' => [
            fn() => [Carbon::now()->subMonth()->startOfMonth()->format('Y-m-d'), Carbon::now()->subMonth()->endOfMonth()->format('Y-m-d')]
        ]
    ];

    foreach ($location_informations as $location_id) {
        // Get date range for the current period
        $dateRange = $dateRanges[$where][0] ?? $dateRanges['today'][0];
        [$startDate, $endDate] = $dateRange();

        $packagesadvances = PackageAdvances::whereBetween('created_at', [$startDate, $endDate])
        ...
        foreach ($packagesadvances as $advance) {
            if (!$this->isValidTransaction($advance)) continue;

            $amount = $advance->cash_amount;

            if ($advance->cash_flow == 'in') {
                $total_revenue += $amount;
            } elseif ($advance->is_refund == 1) {
                $total_refund_out += $amount;
            }
        }
...
private static function isValidTransaction($advance)
{
