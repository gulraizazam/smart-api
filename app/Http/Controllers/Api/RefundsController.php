...
final class RefundsController extends Controller
{
    ...
    // ── Plans for Patient (refund picker) ─────────────────
    ...
    public function plansForPatient(int $patientId): JsonResponse
    {
        ...
        try {
            $plans = Packages::where('patient_id', $patientId)
                ->orderByDesc('id')
                ->get(['id', 'name'])
                ->map(fn ($p) => [
                    'id' => (int) $p->id,
            ...
            Log::error('Plans-for-patient lookup failed', [
    ...
    /**
     * Per-plan refund history for the "View history" dialog.
     * Lists every PackageAdvance row tied to the package along with its
     * payment-mode name and a derived cash-flow label that distinguishes
     * settle/refund out-flows from plain consumption.
     *
     * Returns:
     *   {
     *     patient_name: string,
     *     centre_name:  string,
     *     advances: [{ payment_mode, cash_flow_label, cash_amount, created_at }]
     *   }
     */
    public function history(int $packageId): JsonResponse
    {
        ...
        try {
            ...
            $centre = Locations::find($package->location_id);

            $paymentModes = PaymentModes::pluck('name', 'id');

            $advances = PackageAdvances::where('package_id', $packageId)
                ->where('cash_amount', '!=', 0)
                ->orderBy('created_at', 'asc')
                ->get(['id', 'cash_flow', 'cash_amount', 'payment_mode_id', 'is_refund', 'is_setteled', 'is_cancel', 'is_adjustment', 'created_at']);

            $rows = $advances->map(function ($a) use ($paymentModes) {
                $isRefund = (string) $a->is_refund === '1';
                $isSettled = (string) $a->is_setteled === '1';

                $label = $a->cash_flow;
                if ($a->cash_flow === 'out' && $isRefund) {
                    $label = 'out / refund';
                } elseif ($a->cash_flow === 'out' && $isSettled) {
                    $label = 'out / settled';
                }

                return [
                    'id' => (int) $a->id,
                    'payment_mode' => $paymentModes[$a->payment_mode_id] ?? '—',
                    'cash_flow_label' => $label,
                    'cash_amount' => (float) $a->cash_amount,
                    'created_at' => $a->created_at
                        ? Carbon::parse($a->created_at)->format('M, d Y h:i A')
                        : null,
                ];
            })->values()->all();
            ...
            Log::error('Refund history lookup failed', [
