 ...
 */
    ...
    public function DailyArrival()
    {
        ...
        $cities->prepend('Select a City', '');
        $locations = Locations::getActiveRecordsByCity('',ACL::getUserCentres(), Auth::User()->account_id);
        return view('admin.reports.dailyarrival',compact('locations',  'services',  'cities'));
    }
    ...
    public function LoadDailyArrival(Request $request)
    {
        ...
        ->get();
        $booked = $resultQuery->select('*', 'appointments.name as patient_name', 'appointments.id as app_id', 'appointments.created_by as app_created_by', 'appointments.updated_by as app_updated_by', 'appointments.created_at as app_created_at')
        ->orderBy("appointments.created_at", "DESC")
        ->count();
        $arrived = $resultQuery->select('*', 'appointments.name as patient_name', 'appointments.id as app_id', 'appointments.created_by as app_created_by', 'appointments.updated_by as app_updated_by', 'appointments.created_at as app_created_at')
        ->where('base_appointment_status_id',2)
        ->orderBy("appointments.created_at", "DESC")
        ->count();
        return view('admin.reports.daily_arrived',compact('Appointments','booked','arrived'));
    }
}
