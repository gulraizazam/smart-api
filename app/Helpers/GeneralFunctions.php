    ...
    public static function servicesList($request = null, $total = 0)
    {
        ...
        if ($total >= 0) {
            ...
            foreach ($services as $service) {
                if(Gate::allows('view_inactive_services')){
                    $children = Services::where(['parent_id' => $service->id])->when(hasFilter($filters, 'status'), fn ($q) => $q->where(['active' => $filters['status']]))->orderBy('name')->get()->toArray();
                }else{
                    $children = Services::where(['parent_id' => $service->id,'active'=>1])->when(hasFilter($filters, 'status'), fn ($q) => $q->where(['active' => $filters['status']]))->orderBy('name')->get()->toArray();
