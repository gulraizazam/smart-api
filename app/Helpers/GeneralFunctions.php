...
class GeneralFunctions
{
                    ...
                    $query = Services::with('children')
                        ...
                        foreach ($services as $key => $service) {
                            if($service->parent_id=="0"){
                               $children = Services::where('parent_id',$service->id)->when(isset($where) && count($where) > 0, fn($q) => $q->where($where))->orderBy('name')->get()->toArray();
                               $mergedServices[] = $service->toArray();
                                foreach ($children as $child) {
                                    $mergedServices[] = $child;
                                }
    ...
    {
        $where = [];
        $active = (\Illuminate\Support\Facades\Gate::allows("view_inactive_services")) ? 0 : 1;
        if ($total >= 0 && $id == null) {
            $filename = 'services';
            if(isset($request)){
                $filters = getFilters($request->all());
