...
class PatientReturnCalculator
{
    ...
    /**
     * Calculate "any-doctor" patient return rate for a doctor.
     *
     * Identical to {@see calculate()} except the inner return-existence
     * check omits the `doctor_id = $doctorId` filter — measures whether
     * the patient came back to *anyone in the network* within 45 days,
     * not just this doctor.
     *
     * Used by the management practitioner dashboard to surface "patient
     * leak" — cases where the practice retains the patient but they go to
     * a different doctor.
     *
     * @param int $doctorId
     * @param string $startDate Y-m-d
     ...
     */
    public function calculateAnyDoctor(int $doctorId, string $startDate, string $endDate, int $accountId): array
    {
        $treatmentStatusIds = DoctorDashboardHelper::getTreatmentStatusIds();
        ...
        foreach ($patientTreatments as $patientId => $patientAppts) {
            ...
            foreach ($patientAppts as $treatment) {
                ...
                // Same as calculate() but WITHOUT the doctor_id filter —
                // any arrived treatment within 45 days qualifies as a
                // return for the network as a whole.
                $returnExists = DB::table('appointments')
                    ->where('patient_id', $patientId)
                    ->where('appointment_type_id', 2)
                    ->whereIn('appointment_status_id', $treatmentStatusIds)
